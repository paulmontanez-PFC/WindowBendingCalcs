...
``allowable design stress'' (modulus of rupture / 10). The default allowable
stress is \qty{4.69}{\mega\pascal} (\num{680} psi).
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    ...
    \code{-{}-no-figures}          & off     & Skip only the JPEG figures (still write spreadsheets) \\
...
\begin{table}[h]
  ...
  properties; the allowable stress is the ``allowable design stress'' (modulus of
  rupture / 10) taken from the reference glass and plastics property tables. Verify against
...
renders results with the non-interactive Agg backend and writes JPEG image files
...
appended automatically---see \emph{Output file naming} below). Pass
...
\code{single\_case\_clamped.xlsx}
(a labelled quantity/value/units table); a single-parameter sweep produces
...
The tool always writes its figures as JPEG files (no interactive window). A single
case produces a deflection profile and a stress profile (radial and hoop on both
faces); a sweep produces a two-panel figure of deflection versus the swept
