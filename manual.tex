...
\begin{description}[style=nextline]
  ...
  \item[\code{GridPoint}] One evaluated case of a combined grid sweep: thickness
    (\unit{\milli\meter}), diameter (\unit{\milli\meter}), thickness/diameter ratio,
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    ...
    \code{-{}-sweep-start}         & ---     & Sweep start value \\
    \code{-{}-sweep-stop}          & ---     & Sweep stop value \\
    \code{-{}-sweep-count}         & 7       & Number of sweep samples \\
...
\begin{table}[h]
  ...
  properties. For the glasses the allowable stress is the ``allowable design
  ...
  giving \num{920} psi at \qty{75}{\degree F}. Verify against certified data
  before relying on them for design.}
  \label{tab:materials}
...
on the console. A single case produces \code{single\_case\_deflection\_clamped.jpg}
and \code{single\_case\_stress\_clamped.jpg}; a single-parameter sweep produces
\code{sweep\_\textit{variable}\_clamped.jpg}; the combined sweep produces
\code{sweep\_combined\_clamped.jpg} (the trailing boundary-condition name is
appended automatically---see \emph{Output file naming} below). The figure format
...
\code{single\_case\_deflection\_clamped.jpg} and
\code{single\_case\_deflection\_clamped.svg} (plus the matching stress pair).
...
\code{single\_case\_clamped.xlsx} with two worksheets: \emph{Single Case}
(a labelled quantity/value/units summary table) and \emph{Profile Data},
...
\code{sweep\_\textit{variable}\_clamped.xlsx} (one row per swept value); and the combined
sweep produces \code{sweep\_combined\_clamped.xlsx} with four worksheets. The first,
...
pressure, boundary condition, plate theory, Young's modulus, Poisson's ratio and
allowable stress used for the grid.
...
\paragraph{Output file naming.} Every figure and spreadsheet name always ends
with the boundary condition (\code{\textit{name}\_clamped},
\code{\textit{name}\_simply\_supported}) so results are self-identifying and
runs with different edge conditions never collide. The tool also never
overwrites an existing output file: if the boundary-condition name is already
taken, a numeric suffix is added (\code{\textit{name}\_clamped\_1},
\code{\textit{name}\_clamped\_2}, \ldots) to guarantee uniqueness. The
console always reports the exact path written.
...
\code{-{}-figure-name} and \code{-{}-excel-name}. The boundary-condition suffix,
uniqueness suffix and file extension are still appended, so
\code{-{}-excel-name results} writes \code{results\_clamped.xlsx}. In single-case
mode the two figures take a plot-type suffix, so \code{-{}-figure-name mywindow}
writes \code{mywindow\_deflection\_clamped.jpg} and
\code{mywindow\_stress\_clamped.jpg}. For example,
\begin{quote}
\code{python window\_deformation.py -{}-figure-name mywindow -{}-excel-name results}
\end{quote}
names the default single case \code{mywindow\_deflection\_clamped.jpg},
\code{mywindow\_stress\_clamped.jpg} and \code{results\_clamped.xlsx}.
...
in a single figure (\code{sweep\_combined\_clamped.jpg}) as \emph{families} of curves. The
...
whole grid shares one loading condition. Explicit
\code{-{}-sweep-start}/\code{-{}-sweep-stop}/\code{-{}-sweep-count} values are
ignored in this mode. The console prints three matrices (deflection, stress and
safety factor) with thickness as rows and diameter as columns.

\paragraph{Held-constant annotations.} Every figure states which parameters are
held fixed. The single-case deflection and stress figures carry a subtitle listing
the full input set (e.g.\ \emph{Diameter = 450.0 mm, Thickness = 25.0 mm,
Pressure = 101,000 Pa, Edge = clamped}), and the console output additionally names
the resolved plate theory and the $t/D$ ratio. A single-parameter sweep carries a
figure title listing all fixed quantities (e.g.\ \emph{Held constant:
Diameter = 450.0 mm, Pressure = 101,000 Pa, Edge = clamped, Theory = Kirchhoff}).
The combined figure places the globally common constants (pressure, edge support
and plate theory) in the figure title; each panel's legend identifies the curve
family (diameter for the thickness panels, thickness for the diameter panels).
...
\code{figures/single\_case\_deflection\_clamped.jpg} and
\code{figures/single\_case\_stress\_clamped.jpg}:
...
Saved figure: figures\single_case_deflection.jpg
Saved figure: figures\single_case_stress.jpg
\end{lstlisting}
...
\noindent Produces an aligned table (and writes \code{figures/sweep\_thickness\_mm\_clamped.jpg}):
...
\noindent Produces (and writes \code{figures/sweep\_diameter\_mm\_clamped.jpg}):
...
(\code{figures/sweep\_combined\_clamped.jpg}) is written:
...
\begin{description}[style=nextline]
  \item[Linear center deflection] $w_{0}^{\text{lin}}$ from
    Eq.~\eqref{eq:clamped-linear} or~\eqref{eq:ss-linear}; a quick sanity baseline.
